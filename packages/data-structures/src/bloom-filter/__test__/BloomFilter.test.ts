...
describe('BloomFilter', () => {
  ...
  it('should have methods named "insert" and "mayContain"', () => {
    ...
    expect(typeof bloomFilter.mayExist).toBe('function')
  })
  ...
  it('should create an array with 3 hash values', () => {
    expect(bloomFilter.hashValues('abc').length).toBe(3)
    expect(bloomFilter.hashValues('abc')).toEqual([66, 63, 54])
  })
  ...
  it('should insert strings correctly and return true when checking for inserted values', () => {
    ...
    expect(bloomFilter.mayExist('Bruce Wayne')).toBe(true)
    expect(bloomFilter.mayExist('Clark Kent')).toBe(true)
    expect(bloomFilter.mayExist('Barry Allen')).toBe(true)

    expect(bloomFilter.mayExist('Tony Stark')).toBe(false)
  })
})
